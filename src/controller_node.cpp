...
public:
  ...
  {
    lookahead_min_  = declare_parameter("lookahead_min",  0.35);
    lookahead_max_  = declare_parameter("lookahead_max",  1.20);
    ...
    goal_tol_pos_   = declare_parameter("goal_tolerance", 0.10);
    goal_tol_yaw_   = declare_parameter("heading_tol",    0.15);
    ...
    cmd_pub_ = create_publisher<geometry_msgs::msg::Twist>("/turtlebot3_waffle_pi/cmd_vel", 10);
...
private:
  ...
  void controlLoop()
  {
    ...
    const double yaw_goal  = tf2::getYaw(goal_pose.pose.orientation);

    /* goal handling */
    double yaw_err = normalize(yaw_goal - ryaw);
    if (dist_goal < goal_tol_pos_)
    {
      ...
      else
      {
        ...
        rot.angular.z = std::clamp(yaw_err, -max_yaw_rate_, max_yaw_rate_);
        cmd_pub_->publish(rot);
        return;
      }
    }
    ...
    if (dist_goal < 0.25) linear *= 0.4;
    if (dist_goal < 0.05) linear = 0;
